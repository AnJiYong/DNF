...
public:
	...
	{
		float4 CalVector = { _value , _value , _value , 1.f };
	...
	{
		float4 Value = float4(_Value, _Value, _Value, 1.f);
	...
	unsigned int ColorToUint() const
	{
		...
		Color255[0] = static_cast<char>(x * 255.f);
		Color255[1] = static_cast<char>(y * 255.f);
		Color255[2] = static_cast<char>(z * 255.f);
		Color255[3] = static_cast<char>(w * 255.f);
...
public:
	...
	{
		DirectMatrix = DirectX::XMMatrixScalingFromVector(float4(_Value, _Value, 1.f).DirectVector);
	}
