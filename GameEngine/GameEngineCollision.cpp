...
void GameEngineCollision::Collision(CollisionType _ThisType, CollisionType _OtherType, int _OtherGroup, std::function<void(GameEngineCollision*)> _CallBack, CollisionCheck _Check)
{
	...
	for (GameEngineCollision* OtherCollision : Group)
	{
		...
		if (_Check == CollisionCheck::Begin && true == CollisionCheck_)
		{
			continue;
		}

		CollisionCheck_ = true;
