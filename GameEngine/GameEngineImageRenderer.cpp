...
GameEngineImageRenderer::Animation2D::Animation2D()
	: FolderTextures_(nullptr)
	, AnimationTexture_(nullptr)
	, InterTime_(0.f)
	, CurTime_(0.f)
	, IsEnd(false)
	, Loop_(false)
...
void GameEngineImageRenderer::Update(float _DeltaTime)
{
	...
	if (nullptr != CurAnimation_)
	{
		CurAnimationTexture = CurAnimation_->FolderTextures_->GetTextureIndex(CurAnimation_->CurFrame_);

		if ("Dummy" == CurAnimation_->GetName())
		{
			return;
		}

		float4 Size = CurAnimationTexture->GetTextureSize();

		if (true == Reverse)
		{
			Size.x *= -1;
			GetTransform()->SetLocalScaling(Size);
		}
		else
		{
			GetTransform()->SetLocalScaling(Size);
...
void GameEngineImageRenderer::SetImage(const std::string& _ImageName, const std::string& _Sampler)
{
	...
	GetTransform()->SetLocalScaling(CurTexture->GetTextureSize());
...
void GameEngineImageRenderer::CreateAnimationFolder(const std::string& _Name, const std::string& _FolderTexName, float _InterTime, int _Cur, int _End, bool _Loop)
{
	std::map<std::string, Animation2D*>::iterator FindIter = AllAnimations_.find(_Name);
	...
	NewAnimation->FolderTextures_ = FolderTexture;
	NewAnimation->CurFrame_ = _Cur;
	NewAnimation->EndFrame_ = _End;
	NewAnimation->StartFrame_ = _Cur;
	NewAnimation->Renderer = this;

	if (_End < _Cur)
	{
		NewAnimation->CurFrame_ = _End;
	}
...
void GameEngineImageRenderer::SetChangeAnimation(const std::string& _Name, int _Frame, bool _IsForce)
{
	On();
	...
	{
		ShaderHelper.SettingTexture("Tex", CurAnimation_->FolderTextures_->GetTextureIndex(_Frame));
	}
...
void GameEngineImageRenderer::AnimaOff()
{
	Off();
	CurAnimation_ = nullptr;
}
